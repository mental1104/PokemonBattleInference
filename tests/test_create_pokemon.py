...
if __name__ == "__main__":
    ...
    pokemon = director.construct_from_database(
        id=6, basepoint=(4, 0, 0, 252, 0, 252), nature=Nature.TIMID
    )
    print(pokemon)
    ...
    for _, property in PropertyEnum.__members__.items():
        ...
        result.append(
            AbilityCalculatorFactory.get(property).get_basepoint(
                pokemon.level, ability, speices, individual, pokemon.nature
            )
        )
    print(result)
    print(pokemon.stat)
