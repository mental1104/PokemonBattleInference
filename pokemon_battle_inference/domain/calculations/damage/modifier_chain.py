...
from pokemon_battle_inference.domain.models.types import TypeHelper
from pokemon_battle_inference.domain.models.damage_calculator import DamageResult
...
class BasicDamageModifier(BaseDamageChain):
    ...
    def handle(self, result: DamageResult) -> DamageResult:
        if self.move.move_type in MoveType.get_attack_move():
            attack = (
                self.attacker.stat.attack
                if self.move.move_type == MoveType.physical_move
                else self.attacker.stat.special_attack
            )
            defense = (
                self.defenser.stat.defense
                if self.move.move_type == MoveType.physical_move
                else self.defenser.stat.special_defense
            )
            damage = (
                2 * self.attacker.level + 10
            ) / 250.0 * attack / defense * self.move.power + 2
...
class RandomModifier(BaseDamageChain):
    ...
    def handle(self, result: DamageResult) -> DamageResult:
        ...
        result.random_damage_percent = (
            self.result.random_damage / self.defenser.stat.hp * 100000 // 10 / 100
        )
        return result
...
class TypeStatModifier(BaseDamageChain):
    ...
    def handle(self, result):
        if self.move.move_type.get_attack_move() and (
            self.attacker.type_1 == self.move.type
            or self.attacker.type_2 == self.move.type
        ):
            result *= 1.5
        return result
...
class TypeEfficiencyModifier(BaseDamageChain):
    ...
    def handle(self, result):
        type_multiplier = 1.0
        type_multiplier *= (
            TypeHelper.get_type_efficacy(self.move.type, self.defenser.type_1) / 100.0
        )
        if self.defenser.type_2:
            type_multiplier *= (
                TypeHelper.get_type_efficacy(self.move.type, self.defenser.type_2)
                / 100.0
            )
...
class PercentModifier(BaseDamageChain):
    ...
    def handle(self, result):
        ...
        result.min_damage_percent = percent_decimal_places(
            result.min_damage / self.defenser.stat.hp
        )
        result.max_damage_percent = percent_decimal_places(
            result.max_damage / self.defenser.stat.hp
        )
        result.random_damage_percent = percent_decimal_places(
            result.random_damage / self.defenser.stat.hp
        )
        return result
